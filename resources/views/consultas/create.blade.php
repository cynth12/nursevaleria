...
@section('content')
    <form action="{{ route('consultas.store', $patient->id) }}" method="POST">
        @csrf
        <div class="card">
            <div class="card-body">
                <h4 class="mb-3"></h4>
                ...
                <div class="form-group">
                    ...
                    <label><input type="checkbox" name="symptoms[]" value="Ninguno"
                            {{ str_contains($patient->symptoms, 'Ninguno') ? 'checked' : '' }}> None of the
                        above</label>
                </div>
                ...
                <div class="form-group">
                    <select name="iv_type" class="form-control custom-select">
                        ...
                        <option value="Wellness Duo" {{ $patient->iv_type == 'Wellness Duo' ? 'selected' : '' }}>IV
                            Wellness Duo</option>
                        <option value="Energy Boost" {{ $patient->iv_type == 'Energy Boost' ? 'selected' : '' }}>IV
                            Energy
                            Boost</option>
                        <option value="Beauty Glow" {{ $patient->iv_type == 'Beauty Glow' ? 'selected' : '' }}>IV
                            Beauty
                            Glow</option>
                        <option value="Hangover" {{ $patient->iv_type == 'Hangover' ? 'selected' : '' }}>IV Hangover
                        </option>
                        <option value="Immune Boost" {{ $patient->iv_type == 'Immune Boost' ? 'selected' : '' }}>IV
                            Immune
                            Boost</option>
                        <option value="Immune master Boost"
                            {{ $patient->iv_type == 'Immune master Boost' ? 'selected' : '' }}>IV Immune master Boost
                        </option>
                        <option value="Superdetox" {{ $patient->iv_type == 'Superdetox' ? 'selected' : '' }}>IV
                            Superdetox
                        </option>
                        <option value="Sportpower" {{ $patient->iv_type == 'Sportpower' ? 'selected' : '' }}>IV
                            Sportpower
                        </option>
                        <option value="Post op" {{ $patient->iv_type == 'Post op' ? 'selected' : '' }}>IV Post op
                        </option>
                ...
                <div class="form-group">
                    ...
                    <input type="datetime-local" name="registration_date" class="form-control"
                        value="{{ \Carbon\Carbon::parse($patient->registration_date)->format('Y-m-d\TH:i') }}">
                </div>
            </div>
        </div>
        ...
        <a href="{{ route('consultas.index', $patient->id) }}" class="btn btn-secondary mt-3">
            ⬅️ Return to list
        </a>
