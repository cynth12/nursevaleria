    ...
    public function publicStore(Request $request, $public_token)
{
    ...
    $data['vitamins_intolerance'] =
        $request->vitamins_intolerance === 'yes' ? 1 : 0;

    $data['minerals_intolerance'] =
        $request->minerals_intolerance === 'yes' ? 1 : 0;

    $data['consent_accepted'] =
        $request->consent_accepted === 'yes' ? 1 : 0;
    ...
    return redirect()
        ->route('group.public.form', $group->public_token)
