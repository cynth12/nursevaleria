    ...
    public function model(array $row)
{
    ...
    if (!empty($row['date_of_birth'])) {
        try {
            $dateBirth = Date::excelToDateTimeObject(
                $row['date_of_birth']
            )->format('Y-m-d');
        } catch (\Exception $e) {
            $dateBirth = null;
    ...
    $patient = Patient::create([
        ...
        'email' => !empty($row['email'])
            ? $row['email']
            : 'importado_' . uniqid() . '@nursevaleria.com',
        ...
        'pregnant' =>
            strtolower(trim($row['pregnant'] ?? '')) === 'yes',
        ...
        'vitamins_intolerance' =>
            strtolower(trim($row['vitamins_intolerance'] ?? '')) === 'yes',

        'minerals_intolerance' =>
            strtolower(trim($row['minerals_intolerance'] ?? '')) === 'yes',
        ...
        'consent_accepted' =>
            strtolower(trim($row['consent_accepted'] ?? 'yes')) === 'yes',
    ]);
